...
- **Design before frontend code** (owner rule, 2026-09-26). Any frontend work (`apps/web`, `apps/landing`, `packages/ui`) starts with a design of the part being built, never with code:
  ...
  3. Upload to the one Stitch project `330652592731776730` (https://stitch.withgoogle.com/projects/330652592731776730). Create a new Stitch project only when truly needed, and ask the owner first.
  4. The owner approves or rejects. Approved: start coding. Rejected: design it again and upload it with the next version label ("v2", "v3" …) in the title. Never delete old versions; the owner removes them by hand.
