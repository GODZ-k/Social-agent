...
- `src/config/constants.ts` exports `config`, one object holding the constants used across the API (grouped: `config.crypto`, `config.oauth`, ...). Add to it instead of scattering module-level constants: limits, timeouts, budgets, endpoints and user-facing messages all live there (`config.scan`, `config.firecrawl`, `config.brandAnalyst`, ...). The exception is a parser's own tables (regexes, word lists, CSS selectors, schema.org type sets in `src/scan`): they are the parsing logic, so they stay beside the code that reads them. A repository is one static class: its query helpers (scope filters, `byId`) are `private static` methods called as `XRepository.helper()`, never loose module functions. Types only the API uses live in `src/types` (repository scopes in `src/types/scope.ts`); any type the web app could read (API shapes, query values such as `connect_error`) goes in `packages/shared`: the zod schema in `src/schema`, its `z.infer` type in `src/types`. Comments stay short and say why, not what.
- `src/config/env.ts` validates the environment at start-up and exports `env`. `src/config/db.ts` exports the one database connection, `db`; only repositories import it. A single-row read is `db.query.<table>.findFirst({ where, orderBy })`; an insert that must return its row goes through `insertedRow()` (`src/utils`). A service that works on one brand starts with `BrandsService.findRow(user, brandId)`, the one place that applies the ownership rule and answers 404; never write another lookup.
...
- The ownership rule is `scopeFor` in `src/services/brands.service.ts`: admins reach every brand, everyone else only their own, and "not yours", "archived" and "missing" all answer 404. The repository puts that scope in the WHERE clause of every read and plain update (`reachable` in `brands.repository.ts`); the two questionnaire writes run only after the service's `BrandsService.findRow` check, so they take just the id. `DELETE /brands/:id` archives (`archived_at`); brands are never deleted. New brand-scoped data must take a scope the same way.
...
- There is no automated test suite yet. Every file used for testing lives in `apps/api/testing` (owner rule 2026-09-25): lasting tools for testing each agent and workflow sit in the folder itself, each opening with what it does, how to run it and the output to expect, and with a row in `testing/README.md`; throwaway files (snapshots, one-off checks) go in `testing/temp/` and are deleted when the work is done. Nothing outside `testing` may import from it, so deleting the whole folder leaves the app building, linting and running. Imports use `@/`, never `../src/`.
...
- An agent that must answer in one model call gets its skill text through `loadSkill()` from `@social-agent/agents`, not through `skills:`, which adds tool calls. Skills live in `packages/agents/skills`; the build copies them to `dist/skills`. Agents and skills are reused outside Cadence, so nothing in `packages/agents` may import app code.
