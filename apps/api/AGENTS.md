...
- **Every endpoint is an operation in `src/openapi/operations.ts`. Adding or changing an endpoint is not done until that entry is updated and `pnpm --filter api run openapi` passes.** The script regenerates `apps/api/openapi/openapi.json` (committed, never edited by hand) and fails when a route in `src/routes` has no operation or an operation has no route (`openapi:check` runs the check alone). The interactive reference is `/api/docs` in development (Scalar, with "try it"); the committed JSON is the contract otherwise. A Clerk token for trying endpoints: `pnpm --filter api run dev-token -- <email>` (needs `CLERK_SECRET_KEY` in `.env`).
