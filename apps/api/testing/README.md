...
| [`discovery.ts`](discovery.ts) | Business discovery on a real brand from the database | `pnpm --filter api run discovery -- <brandId>` | Database, Anthropic, Firecrawl; the brand's intake approved | Yes, about 3 to 5 min per run |
